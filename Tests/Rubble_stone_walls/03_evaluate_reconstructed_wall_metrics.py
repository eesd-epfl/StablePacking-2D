"""
Evaluate interlocking, filling, stability of generate walls.
Input:
    - data_dir_prefix: the prefix of the directory that contains the transformation.txt file
    - result_dir: the directory that will store metrics.txt
...
result_dir = "./result_20231114-194339/"
#write metrics
...
for i in range(12,13):
    ...
    # remove all black pixels rows on the top
    wall_eroded_stones = wall_eroded_stones[0:np.max(np.argwhere(wall_eroded_stones!=0)[:,0]),:]
