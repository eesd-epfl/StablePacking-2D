...
set_world_size(int(img_size[1]*_rescale)-15, int(img_size[0]*_rescale)+5)
...
nb_clusters = vendor.cluster_stones(plot_figure = True,plot_cluster_txt = False)
...
plt.savefig(get_dir()+"img/original_wall_with_id.pdf", dpi=300)

###############################################################original wall without id
plt.close()
original_wall = np.zeros((wall_img.shape))
text_kwargs = dict(ha='center', va='center',
                           fontsize=12, color='black')
...
ax.axes.xaxis.set_visible(False)
ax.axes.yaxis.set_visible(False)
plt.savefig(get_dir()+"img/original_wall_without_id.pdf", dpi=300)
...
newcolors[0] = np.asarray([1,1,1,0])
newcmp = ListedColormap(newcolors, name='OrangeBlue')
plt.clf()
f = plt.figure(dpi=3000)
...
plt.savefig(_root_dir+"/result/img/wall_stone_id_noscale.png", transparent = True)
...
plt.close()
fig, ax = plt.subplots()
# change_dict = {4:4,3:3,2:2,1:1,0:0,-1:-1}
...
ax.imshow(plt_matrix, cmap=newcmp,interpolation='none',vmax = 7)
text_kwargs = dict(ha='center', va='center',
                    fontsize=12, color='black')
