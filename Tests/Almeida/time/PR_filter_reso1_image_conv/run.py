...
dir_path = _root_dir+'/data_clustered/'
labels_ = []
ids = []
for filename in os.listdir(dir_path):
    ...
    if os.path.isfile(os.path.join(dir_path, filename)):
        ...
        # read png binary img
        stone_matrix = np.flip(cv2.imread(os.path.join(dir_path, filename))[:, :, 0], axis=0)
        stone = Stone()
        _,_ = stone.from_matrix(stone_matrix,scale = _rescale)
        # parse the file name
        stone.id = int(filename.split('.')[0].split('_')[1])
        stone.cluster = int(filename.split('.')[0].split('_')[2])
        stones.append(stone)
        labels_.append(stone.cluster)
        ids.append(stone.id)
# sor labels by ids
ids = np.asarray(ids)
labels_ = np.asarray(labels_)
labels_ = labels_[ids.argsort()]
...
while(run_counter < 1):
    ...
    for i in range(nb_clusters):
        vendor.labels[i] = np.argwhere(labels_ == i)[:,0]
        print(vendor.labels[i])
    #nb_clusters = vendor.cluster_stones()

    # final_base = build_wall_vendor(
    #     base, vendor, int(1.3*nb_clusters))
    final_base = build_wall_vendor(
        base, vendor, nb_clusters,vendor_type = 'variant',construction_order='from_left')
