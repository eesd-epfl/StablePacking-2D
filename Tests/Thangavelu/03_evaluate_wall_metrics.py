...
Input:
    ...
    - noise_level: the noise level of the dataset
...
noise_level = 80
#----------------------------------------------------noise 80 with orientation interval 10 degree
data_dir_prefix = "./result_20231120-223433_wall"
noise_level = 80

#write metrics
with open(result_dir+f'metrics_{noise_level}.txt', 'a+') as f:
...
for i in range(20):
    ...
    #remove all zeros columns on the right
    wall_image = wall_image[:,:np.max(np.argwhere(np.sum(wall_image,axis=0)!=0))+1]
